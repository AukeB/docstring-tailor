def generate_windowed_stats(
    ...
    Slides a window of fixed size across the input data, yielding summary
    ...
    Examples:
        Basic usage over a short sequence, stepping one position at a time. The
        loop body uses continuation lines to stay within the line length limit.
        ...
        Increasing the step size reduces the number of windows yielded. The
        multi-line call below uses ellipsis continuation to keep each argument
        on its own line, which is preferred when there are many keyword
        arguments.
        ...
        Value bounds filter out windows whose mean falls outside the desired
        range. Only the middle window survives here since its mean of 3.0 is the
        only one within the bounds of 2.5 and 3.5.
