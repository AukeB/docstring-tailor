def generate_windowed_stats(
    ...
    Args:
        data (list[float]): The input sequence of numerical
            values to process.
        window_size (int): The number of elements in each
            window. Must be greater than zero and no larger
            than the length of data.
        step (int): The number of elements to advance the
            window between each yield. Defaults to 1.
        min_value (float | None): If provided, windows whose
            mean falls below this value are skipped.
            Defaults to None.
        max_value (float | None): If provided, windows whose
            mean exceeds this value are skipped. Defaults to
            None.
    ...
    Yields:
        ...
        mean (float): The arithmetic mean of the values in
            the current window, rounded to precision decimal
            places.
        std_dev (float): The standard deviation of the
